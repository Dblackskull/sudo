...
p = subprocess.Popen(['airodump-ng','-whi', iface], stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)       # line-1.1
...
pp= subprocess.Popen([f'airodump-ng', iface , '--bssid', bsid , '--channel', cha ,'-wmon' ], stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE)              # monitoring specific AP (wifi)
...
def msg(mess):
	...
	payload = f"sender_id=FSTSMS&message={mess}&language=english&route=p&numbers=8667688729"
...
def bruteforce_usercheck(a,b,c,pre_mac,pre_no,pre_ex,count):
	qq=subprocess.Popen([f'airodump-ng', iface , '--bssid', bsid ,'--channel' , cha , '-wmon' ], stdout=subprocess.PIPE, stdin=subprocess.PIPE, stderr=subprocess.PIPE) 
